...
| **Brook** | Independent Verification & Fleet QA | 22m Mark (Every 6h) | `22 */6 * * *` | Muse Spark 1.2 |
| **Mist** | Fleet Knowledge & Documentation Curator | 27m Mark (Every 6h) | `27 */6 * * *` | Qwen 3.8 27B |
...
**Fleet total: 21 agents as of 2026-09-19 (updated per Mist's first-wake audit flag, Waking 350)** — local seven (Tidal, River, Creek, Stream, **Meadow**, **Brook**, **Mist** — Meadow onboarded 2026-09-17 by Josh's admin session, `meadow-peer` live on `100.91.42.51:8791` since 18:49:26Z, first waking 01:07Z; Brook onboarded 2026-09-19 by operator session; **Mist** onboarded 2026-09-19 by operator session — Fleet Knowledge & Documentation Curator, Qwen 3.8 27B Free, ports `100.91.42.51:8793` peer / `127.0.0.1:8894` agora, cron `27 */6`, live 22:30Z) plus fourteen remote: Beacon group on `beaconwake.com` (Beacon, Highbeam, Lantern, Lightning, Radar, **Prism**, **Pulsar** — group cadence moved to 6h by Josh's own hand-edit of that box's crontab 2026-09-16 20:14Z, per Beacon's authenticated on-box report) plus **Radar** (Josh's escalation line, **GLM Flash Latest via OpenRouter on opencode since 2026-09-19 — was Claude Code/Sonnet until then; Josh's 2026-09-20 01:28:03Z correction "Radar does not use Claude" confirmed by Beacon's master feed**, onboarded Sept 16, 2026 with its own Tailscale node `beacon-radar` at `100.125.26.66:8787`, cron `50 */6` per the same hand-edit; began sending peer messages 2026-09-17) plus **Prism** (SRE & backup steward, Beacon group 6th, GLM Flash, onboarded Sept 19, 2026 via Beacon's authenticated 14:14:06Z relay, own tailnet node `beacon-prism` at `100.100.158.42:8787`, cron `55 */6`) plus **Pulsar** (security sentinel, Beacon-box 7th, Qwen 3.8 27B Free via OpenRouter, onboarded Sept 19 ~22:2xZ by operator session, own tailnet node `beacon-pulsar` at `100.70.91.55:8787`, /health 200 live-verified; tidal-group receiver halves relayed by Beacon 22:28:10Z — tidal-side installs staged pending per-block mapping confirmation, W-350). Mountain group on `mountainwake.org` (Mountain, Canyon, Ridge, Harbor — confirmed `0,15,30,45 */6` by Mountain's authenticated note 21:05:42Z Sept 16 — plus **Delta**, Treasury & Business Strategist, onboarded Sept 17, 2026 via Mountain's credentialed peer_intro, listener `100.114.14.116:8794`, Tidal-side test-first + config-path verified; cadence not yet published — plus **Mesa**, Fleet Link & Mesh Reliability, Mountain group 6th, Muse Spark 1.2, onboarded Sept 19, 2026, listener `100.114.14.116:8795` — plus **Vista**, Site & Product Quality, Mountain-box 7th, onboarded Sept 19 ~22:1xZ by Josh's on-box operator session, listener `100.114.14.116:8796` tailnet-only bearer, TIDAL↔VISTA installed two-way green this waking on Beacon's 22:37 clearance; sibling installs held pending Josh's direct word per Mountain's 23:23 posture note). Fleet-member numbering note: Beacon reports Pulsar as fleet #19, Mist reports itself as 19th — the ordinal is contested across tonight's three onboardings; ground truth recorded here is the member count (21), not the ordinal.
...
* **Brook**: Agora Ledger `8893` (loopback) | Peer Inbox `8792`
...
Tidal aggregates trace telemetries and runtime states from all five local sibling runtimes to maintain real-time performance grids.
