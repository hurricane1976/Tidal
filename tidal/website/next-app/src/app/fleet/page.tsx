...
const MEMBERS: Member[] = [
  ...
  { name: "Highbeam", accent: "var(--amber)", status: "Active Remote", model: "Claude Code (Sonnet) | Host: own Tailscale node beacon-highbeam (100.81.147.28) | Link: identity, live", role: "Vulnerability & Code Review", desc: "Conducts deep package reviews, parses vulnerability feeds, runs research loops, and generates architectural hardening strategies for other agents. Listener live; zero-secret identity link live them→us (first test received 23:06Z); the return path awaits a Beacon-side roster entry for gemini-agent." },
  { name: "Lantern", accent: "var(--amber)", status: "Active Remote", model: "GLM 5.3 Flash | Host: own Tailscale node beacon-lantern (100.76.139.96) | Link: identity, live", role: "UI/UX & Visual Assets", desc: "Performs visual rendering diagnostics, verifies responsive web layouts, compiles SVG fleet topologies, and performs multi-model front-end reviews. Listener live; zero-secret identity link to all four local agents is live (them→us) — first sibling link live (Sept 11). Return path awaits a Beacon-side roster entry for gemini-agent." },
  { name: "Lightning", accent: "#ecc94b", status: "Active Remote", model: "DeepSeek V4 Pro | Host: own Tailscale node beacon-lightning (100.69.40.118)", role: "Data Analysis, Metrics & Monitoring", desc: "Performs quantitative fleet and traffic analysis, anomaly detection, resource-trend alerts, and generates periodic digest snapshots published into the shared outbox. Listener live; peer link pending adoption of the relayed identity recipe." },
