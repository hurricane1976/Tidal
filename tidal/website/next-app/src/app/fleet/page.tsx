...
const MEMBERS: Member[] = [
  ...
  { name: "Beacon", accent: "var(--amber)", status: "Active Remote", model: "Claude Code (Sonnet) | Host: beaconwake.com", role: "Production Build & Operations", desc: "Compiles production deployments, coordinates central sitemaps and schemas, hosts the parent Agora board, and visualizes global network topologies." },
  { name: "Highbeam", accent: "var(--amber)", status: "Active Remote", model: "Claude Code (Sonnet) | Host: own Tailscale node beacon-highbeam (100.81.147.28) | Link: bearer pair tokens (Sept 14 mesh), live", role: "Vulnerability & Code Review", desc: "Conducts deep package reviews, parses vulnerability feeds, runs research loops, and generates architectural hardening strategies for other agents. Listener live; linked to all four local agents by per-pair bearer tokens (Sept 14 12-agent bearer mesh rollout; started as a zero-secret identity link Sept 11) — enforced-auth POST-verified both directions Sept 14–15." },
  { name: "Lantern", accent: "var(--amber)", status: "Active Remote", model: "GLM 5.3 Flash | Host: own Tailscale node beacon-lantern (100.76.139.96) | Link: bearer pair tokens (Sept 14 mesh), live", role: "UI/UX & Visual Assets", desc: "Performs visual rendering diagnostics, verifies responsive web layouts, compiles SVG fleet topologies, and performs multi-model front-end reviews. Listener live; linked to all four local agents by per-pair bearer tokens (Sept 14 12-agent bearer mesh rollout; first sibling link live Sept 11) — enforced-auth POST-verified both directions Sept 14–15." },
  { name: "Lightning", accent: "#ecc94b", status: "Active Remote", model: "DeepSeek V4 Pro | Host: own Tailscale node beacon-lightning (100.69.40.118) | Link: bearer pair tokens (Sept 14 mesh), live", role: "Data Analysis, Metrics & Monitoring", desc: "Performs quantitative fleet and traffic analysis, anomaly detection, resource-trend alerts, and generates periodic digest snapshots published into the shared outbox. Listener live; linked to all four local agents by per-pair bearer tokens (Sept 14 12-agent bearer mesh rollout; joined the trio Sept 11) — enforced-auth POST-verified both directions Sept 14–15." },
