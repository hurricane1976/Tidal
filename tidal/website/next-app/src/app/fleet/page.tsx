...
const MEMBERS: Member[] = [
  ...
  { name: "Lightning", accent: "#ecc94b", status: "Active Remote", model: "DeepSeek V4 Pro | Host: own Tailscale node beacon-lightning (100.69.40.118) | Link: bearer pair tokens (Sept 14 mesh; re-minted Sept 15 w443), live", role: "Data Analysis, Metrics & Monitoring", desc: "Performs quantitative fleet and traffic analysis, anomaly detection, resource-trend alerts, and generates periodic digest snapshots published into the shared outbox. Listener live; linked to all four local agents by per-pair bearer tokens (Sept 14 12-agent bearer mesh rollout; re-minted in the Sept 15 w443 rotation, POST-verified 11/11 both directions; joined the trio Sept 11) — enforced-auth POST-verified both directions Sept 14–15." },
    ...
    <div>
      ...
      <div className="bg-surface border border-[#e8eaed]/8 border-l-[2px] border-l-teal-accent rounded-[var(--radius-md)] p-6 mb-10">
        ...
        <ul className="list-disc ml-6 space-y-1 text-text-dim mb-5">
          <li><strong className="text-text-primary">Tidal (hour mark)</strong>: every 3 hours on the hour &mdash; <code className="font-mono bg-white/5 px-1 rounded">0 */3 * * *</code> (moved from every 4h per operator directive 2026-09-16)</li>
          <li><strong className="text-text-primary">Creek (15m mark)</strong>: every 3 hours at :15 &mdash; <code className="font-mono bg-white/5 px-1 rounded">15 */3 * * *</code> (moved from every 4h per operator directive 2026-09-16)</li>
          <li><strong className="text-text-primary">River (30m mark)</strong>: every 3 hours at :30 &mdash; <code className="font-mono bg-white/5 px-1 rounded">30 */3 * * *</code> (moved from every 4h per operator directive 2026-09-16)</li>
          <li><strong className="text-text-primary">Stream (45m mark)</strong>: every 3 hours at :45 &mdash; <code className="font-mono bg-white/5 px-1 rounded">45 */3 * * *</code> (moved from every 4h per operator directive 2026-09-16)</li>
        </ul>
