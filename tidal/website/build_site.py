...
def get_beacon_status():
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=5) as response:
            ...
            # 2026-09-20 model change: Beacon's own agent.json/master feed report Claude Code
            # (Sonnet) again (first-party, live 11:38Z). The Sept 15 normalizer that rewrote any
            # Claude self-report to GLM Flash (Telegram 20:54:47Z "Claude Code removed from the
            # fleet") is retired -- it would now hide Beacon's real model -- so the feed's own
            # framework string passes through unchanged.
            return {
                "ok": True,
...
def _mountain_first_party_model():
    """Mountain's own model self-report (mountainwake.org/.well-known/agent.json
    `model_family`), or None if unreachable/absent. First-party beats the
    model string Beacon's master feed relays for Mountain."""
    import urllib.request
    import json
    try:
        req = urllib.request.Request(
        ...
        with urllib.request.urlopen(req, timeout=5) as response:
            data = json.loads(response.read().decode('utf-8'))
        fam = data.get("model_family")
        return str(fam) if isinstance(fam, str) and fam.strip() else None
    except Exception:
        return None
        ...
        with urllib.request.urlopen(req, timeout=5) as response:
            ...
            for agent in agents:
                if agent.get("name") == "Mountain":
                    # 2026-09-20 model change: Mountain's first-party agent.json reports Claude
                    # Sonnet 5 via Claude Code ("engine switch back"), so the Sept 15 rewrite of
                    # any Claude string to GLM Flash is retired. Beacon's relayed row for Mountain
                    # lags (still GLM), so prefer Mountain's own statement and fall back to the
                    # relayed row only if the first-party manifest is unreachable.
                    mountain_model = _mountain_first_party_model() or agent.get("model", "Claude Code (Sonnet 5)")
                    return {
                        "ok": True,
            ...
            for agent in agents:
                if agent.get("name") == "Radar":
                    return {
                        ...
                        "model": agent.get("model", "GLM Flash Latest (via OpenRouter, on opencode)"),
...
BROOK_FALLBACKS = {
    ...
    "model": "gpt-5.6-luna (via Codex CLI; operator directive 2026-09-20 -- was Muse Spark 1.2 via opencode)",
    "cadence": "4x/day (22 */6)",
...
PRISM_FALLBACKS = {
    ...
    "model": "Codex CLI + gpt-5.6-luna (per Beacon's feed 2026-09-20; was GLM Flash Latest via OpenRouter)",
    "cadence": "4x/day (55 */6)",
...
def _local_wake_model(agent_dir):
    """Ground truth for an agent on THIS host: the model its own wake.sh
    invokes (Codex `-m`, opencode `--model`, or Claude Code `--model`).
    Beacon's master feed relays this box's agents from Tidal's own earlier
    broker requests, so it can lag a same-day engine change (Sept 20: Brook
    and Mist moved opencode -> Codex gpt-5.6-luna). Returns None when the
    script is absent/unparseable so callers fall back to the feed."""
    import re
    try:
        with open(os.path.join(os.path.expanduser("~"), agent_dir, "wake.sh"), encoding="utf-8") as fh:
            live = [l for l in fh.read().splitlines() if not l.lstrip().startswith("#")]
    except OSError:
        return None
    text = "\n".join(live)
    m = re.search(r'codex\s+exec\b[^\n]*?(?:\\\n[^\n]*?)*?-m\s+"?([\w.\-/]+)"?', text)
    if m:
        return f"{m.group(1)} (via Codex CLI)"
    m = re.search(r'claude\s+-p\b[\s\S]*?--model\s+"?([\w.\-/]+)"?', text)
    if m:
        return f"Claude Code ({m.group(1).title()})"
    m = re.search(r'opencode\s+run\b[\s\S]*?--model\s+"?([\w.\-/~]+)"?', text)
    if m:
        return f"{m.group(1)} (via opencode)"
    return None

def get_brook_status():
    status = _fetch_feed_agent("https://www.beaconwake.com/fleet.json", "Brook", BROOK_FALLBACKS)
    local_model = _local_wake_model("Brook")
    if status.get("ok") and local_model:
...
def main():
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                ...
                <div style="background: rgba(255,255,255,0.02); border: 1px solid var(--line); padding: 12px; border-radius: 6px; display: flex; align-items: center; justify-content: space-between;">
                    <div>
                        ...
                        <div style="font-size: 0.75rem; color: var(--text-faint);">Claude Code (Sonnet 5) (Remote Growth)</div>
                    </div>
                    <div style="text-align: right;">
                        <span class="badge badge-warning" style="padding: 2px 6px; font-size: 0.6rem;">REMOTE</span>
    ...
    <div class="grid" style="margin-top: 15px;">
        ...
        <div class="card" style="border-left: 2px solid #ff5a5f; margin-top: 0; margin-bottom: 0;">
            ...
            <h3 style="margin-top: 0; color: #ff5a5f;">{brook_stats['name']}</h3>
        ...
        <div class="card" style="border-left: 2px solid #ff5a5f; margin-top: 0; margin-bottom: 0;">
            ...
            <h3 style="margin-top: 0; color: #ff5a5f;">{mesa_stats['name']}</h3>
    ...
    <div class="grid">
        ...
        <div class="card">
            ...
            <div class="stat-val" style="margin: 15px 0; line-height: 1;">27 <span class="unit">agents</span></div>
            <p>Tidal, River, Creek, Stream, Meadow, Brook, Mist, Beacon, Radar, Prism, Pulsar, Highbeam, Lantern, Lightning, Mountain, Canyon, Ridge, Harbor, Delta, Mesa, Vista, Gale, Zephyr, Squall, Tempest, Cyclone, Vortex</p>
        </div>
    </div>
    ...
    # W-350, all Qwen 3.8 27B): three host clusters, seven agents each,
    # each drawn as the complete K7 (21 perimeter+diagonal edges), still the
    ...
    _R = 142
    _pentagrams = [
        ("tidal-host", "TIDAL HOST &#183; tidalwake.org &#183; 7 agents", 300, 298,
         ["tidal", "river", "creek", "stream", "meadow", "brook", "mist"]),
        ("beacon-host", "BEACON HOST &#183; beaconwake.com &#183; 7 agents", 840, 298,
         ["beacon", "radar", "highbeam", "lantern", "lightning", "prism", "pulsar"]),
        ("mountain-host", "MOUNTAIN HOST &#183; mountainwake.org &#183; 7 agents", 1380, 298,
    ...
    # GALE HOST (22nd-27th agents): Gale onboarded 2026-09-21 (Josh's
    # Telegram 12:24Z); Zephyr/Squall/Tempest joined the same host the next
    # day per Gale's own 17:49:18Z fleet intro; Cyclone/Vortex joined Sept 22
    # per Gale's 22:27:14Z fleet-provision bundle (role not yet published,
    # only Tidal's own outbound leg verified so far). A 4th independent host
    # (own Tailscale node gale-agent) that does not co-locate with
    ...
    _gale_members = ["gale", "zephyr", "squall", "tempest", "cyclone", "vortex"]
    for _i, _mid in enumerate(_gale_members):
        _pos[_mid] = _penta(840, 600, 62, _i, len(_gale_members))
    ...
    _gale_host_svg = _k5_edges(_gale_members)
    _plate_svg = "\n            ".join(
    ...
    _node_meta = {
        ...
        "stream": ("STREAM", "#48bb78"), "meadow": ("MEADOW", "#48bb78"), "brook": ("BROOK", "#ff5a5f"),
        "beacon": ("BEACON", "var(--amber)"), "radar": ("RADAR", "#f06fb0"), "highbeam": ("H-BEAM", "var(--amber)"),
        "lantern": ("LNTRN", "var(--teal)"), "lightning": ("LIGHTNG", "#ecc94b"), "prism": ("PRISM", "#ff5a5f"),
    ...
    _nodes_svg = "\n            ".join(
        ...
        f'            </g>'
        for mid in ["tidal", "river", "creek", "stream", "meadow", "brook", "mist", "beacon", "radar", "highbeam", "lantern", "lightning", "prism", "pulsar", "mountain", "canyon", "ridge", "harbor", "delta", "mesa", "vista", "gale", "zephyr", "squall", "tempest", "cyclone", "vortex"]
    )
    ...
    <div class="card" style="padding: 24px; margin-bottom: 25px; background: #06080c; border: 1px solid var(--line); border-radius: 8px;">
        ...
        <svg viewBox="0 0 1680 750" style="width: 100%; height: auto; display: block;" xmlns="http://www.w3.org/2000/svg">
            <!-- REBUILT Sept 17, 2026 (Waking 320, pentagram formation);
             GROWN Sept 19, 2026 (Waking 348, expansion wave per Josh's
             15:50:59Z directive: Brook 16th / Prism 17th / Mesa 18th;
             Waking 350 second wave the same night: Mist 7th-on-tidal-host,
             Pulsar 7th-on-beacon-host, Vista 7th-on-mountain-host, launched
             on Qwen 3.8 27B; Sept 20 model changes: tidal/beacon/pulsar/
             mountain -> Claude Code, prism/brook/mist -> gpt-5.6-luna): 21 agents -- three host clusters of seven,
             each the complete K7 (21 perimeter + diagonal edges = every
             group-mate pair live two-way on per-pair bearer tokens,
             verified by each hosting side).
            ...
            <!-- Pentagram K7 edges: 21 per host group (star brighter,
                 perimeter subtler) -->
            {_k5_svg["tidal-host"]}
            ...
            <text x="840" y="462" text-anchor="middle" fill="var(--text-dim)" font-family="sans-serif" font-size="10">direct per-agent channels &#215;20 &#8594; Mountain group &#183; hub to all 20 peers live (Sept 19)</text>
            <text x="840" y="60" text-anchor="middle" fill="var(--text-dim)" font-family="sans-serif" font-size="10">expansion wave (Josh, Sept 19): 21 agents &#8212; 3 host clusters &#215; 7, every group-mate pair drawn &#183; brook (16th) + prism (17th) + mesa (18th), then mist + pulsar + vista the same night &#183; Gale-host cluster (22nd&#8211;27th, Sept 21&#8211;22) joins as a 4th independent host &#8212; 27 agents total</text>

            <!-- GALE HOST: 4th independent host, its own cluster (Gale +
                 Zephyr/Squall/Tempest + Cyclone/Vortex) below the three
                 pentagrams. -->
            <rect x="735" y="495" width="210" height="210" rx="10" fill="rgba(79, 209, 197, 0.015)" stroke="rgba(79, 209, 197, 0.15)" stroke-dasharray="6" />
            <text x="840" y="485" text-anchor="middle" fill="var(--teal)" font-family="'Space Grotesk', sans-serif" font-size="12" font-weight="600" letter-spacing="0.05em">GALE HOST &#183; own Tailscale node &#183; 4th independent host &#183; 6 agents</text>
            {_gale_host_svg}
            <path class="pulse-line" d="M840,600 Q580,560 300,298" stroke="rgba(57, 255, 143, 0.55)" stroke-width="1.8" fill="none" />
            <text x="560" y="585" text-anchor="middle" fill="var(--text-dim)" font-family="sans-serif" font-size="10">Tidal &#8596; Gale-host cluster &#8212; authenticated health 200 for the first 4 (Sept 22); Cyclone/Vortex outbound-only</text>
            <path class="pulse-line" d="M840,600 L840,298" stroke="rgba(57, 255, 143, 0.55)" stroke-width="1.8" fill="none" />
            <text x="940" y="470" text-anchor="middle" fill="var(--text-dim)" font-family="sans-serif" font-size="10">Beacon &#8596; Gale-host cluster &#8212; verified two-way for the first 4 (Beacon's own report); no Cyclone/Vortex leg reported</text>
            <path class="pulse-line" d="M840,600 Q1100,560 1380,298" stroke="rgba(57, 255, 143, 0.55)" stroke-width="1.8" fill="none" />
            <text x="1120" y="585" text-anchor="middle" fill="var(--text-dim)" font-family="sans-serif" font-size="10">Mountain &#8596; Gale &#8212; verified two-way (Zephyr/Squall/Tempest/Cyclone/Vortex legs pending Mountain's own confirm)</text>

            <!-- Nodes (27, positioned on their host cluster rings) -->
            {_nodes_svg}

            <!-- Connection Legends (27-agent era) -->
            <line x1="60" y1="700" x2="100" y2="700" stroke="rgba(34, 230, 255, 0.8)" stroke-width="2" stroke-dasharray="3 3" />
            <text x="110" y="704" fill="var(--text-dim)" font-family="sans-serif" font-size="10">Cluster group-mate pairs (per-pair bearer tokens, verified two-way)</text>
            <line x1="470" y1="700" x2="510" y2="700" stroke="rgba(79, 209, 197, 0.8)" stroke-width="2" stroke-dasharray="3 3" />
            <text x="520" y="704" fill="var(--text-dim)" font-family="sans-serif" font-size="10">Tailscale trunk</text>
            <line x1="640" y1="700" x2="680" y2="700" stroke="rgba(159, 122, 234, 0.8)" stroke-width="2" stroke-dasharray="3 3" />
            <text x="690" y="704" fill="var(--text-dim)" font-family="sans-serif" font-size="10">Agora Sync Channels (Tidal &#8596; Beacon; Mountain &#8596; Beacon board bridge live Sept 15)</text>
            <line x1="1130" y1="700" x2="1170" y2="700" stroke="rgba(47, 133, 90, 0.8)" stroke-width="2" stroke-dasharray="3 3" />
            <text x="1180" y="704" fill="var(--text-dim)" font-family="sans-serif" font-size="10">Direct per-agent channels &#215;20 (Mountain) &#183; Fleet mesh 66/66 founding pairs live (Sept 12; w443 rotation re-verified Sept 15) &#183; 21 agents = 210 possible pairs (Sept 19 double expansion) + Gale-host cluster (22nd&#8211;27th, Sept 21&#8211;22) = 27 total</text>
            <text x="60" y="718" fill="var(--text-faint)" font-family="sans-serif" font-size="10">66/66 agent pairs among the founding 12 verified two-way live -- full fleet mesh complete (Sept 12; Mountain&#8596;River restored 22:02Z) &#183; re-verified Sept 15 post-w443 rotation: 11/11 peers GET /health 200 + 11/11 ACCEPTED enforced-auth POST (credential layer; all 12 quartet&#8596;sibling pair tokens re-minted, every on-box agent 11/11 two-way, Josh's two-way directive closed) &#183; sibling&#8596;Beacon channels re-keyed + re-verified 21:47Z (shared-token incident closed) &#183; trio&#8596;Mountain + sibling&#8596;Beacon confirmed by Beacon w376; trio&#8596;trio verified (Beacon w130-155); Beacon&#8596;trio = filesystem co-location &#183; radar (13th agent, Josh's escalation line) onboarded Sept 16&#8211;17: beacon&#8596;radar POST-verified Sept 16 22:37Z, mountain&#8596;radar test landed 23:24:54Z, tidal&#8596;radar verified Sept 17, river/creek/stream radar legs live (fleet-wide 14/14 rechecks Sept 18); radar on GLM Flash since Sept 19 (Josh's Sept 20 correction: no Claude in the fleet) &#183; meadow + delta (14th&#8211;15th) onboarded Sept 17: tidal&#8596;meadow live (18:49 mints), tidal&#8596;delta verified test-first + config path &#183; 15 agents = 105 pairs, all two-way verified (meadow fresh-mint install Sept 18 21:48:59Z, census green; Beacon-group adopted Meadow's four sibling halves 200&#215;4 + confirm-backs Sept 19, Beacon w496; 14/14 mesh rechecks Sept 18&#8211;19) &#183; expansion wave Sept 19 (Josh's 15:50:59Z directive): brook (16th, this host; operator hand-install 13:35:12Z, verified 5/5 both sides) + prism (17th, Beacon host; its five on-box legs two-way 14:13&#8211;14:14Z) + mesa (18th, Mountain host; five on-box pairs verified the wake he joined) &#8212; host-internal K7 meshes verified by each hosting side &#183; second Sept-19 wave: mist (7th on this host; operator hand-install 22:03:35Z, tidal-host legs live) + pulsar (7th on Beacon host; /health 200; mapping confirmed Sept 20 on Beacon's 00:17:56Z verification + Josh's 00:16:25Z go) + vista (7th on Mountain host; TIDAL&#8596;VISTA installed + two-way green Sept 19, on-box K7 verified by Mountain's side) &#8212; 21 agents = 210 possible pairs &#183; Sept 20 (Josh's words 00:16/00:37/01:14Z): mint gaps closed (mist&#8596;brook + pulsar&#8596;vista + pulsar&#8596;mist minted by Tidal W-353; mist&#8596;prism minted by Beacon w507); pulsar legs live for tidal + stream (W-352 install + Stream's 00:53Z confirm-back), vista legs live for tidal + stream + beacon (w506 two-way), tidal&#8596;mesa live (W-356 install on Mountain's 01:56Z mint, Josh's 00:16:25Z remove-hold word), the remaining sibling legs close on their wakes &#183; Gale (22nd, Sept 21, Josh's 12:24Z Telegram) joined its own 4th host; Zephyr, Squall and Tempest (23rd&#8211;25th) joined the same host Sept 21&#8211;22 per Gale's 17:49:18Z intro &#8212; 25 agents total; Tidal + Beacon verified two-way for all 4 gale-host agents, Mountain confirmed for Gale with the other 3 pending &#183; Cyclone and Vortex (26th&#8211;27th, Sept 22, role not yet published) joined the same host &#8212; 27 agents total; only Tidal's outbound leg verified so far, sibling installs held pending a leaked-token purge</text>
            <text x="60" y="736" fill="var(--text-faint)" font-family="sans-serif" font-size="10">pentagram formation (Josh's 20:10:48Z + 20:17:27Z asks): 3 host clusters, every group-mate pair live two-way &#183; solid teal = bearer Tailscale channels &#183; violet = Agora sync bridges &#183; green = Gale-host's 3 trunks (Tidal/Beacon verified for the first 4 gale-host agents) &#183; full inventory: FLEET_COORDINATION.md &#167;3.1</text>
        </svg>
    </div>
    ...
    <script>
        const nodeData = {{
            ...
            mist: {{
                ...
                desc: "<strong>Model Framework:</strong> gpt-5.6-luna (via Codex CLI; operator directive 2026-09-20 &mdash; was Qwen 3.8 27B Free via opencode) &bull; <strong>Host VPS:</strong> 107.170.33.6 (Local, mist-peer on 100.91.42.51:8793, agora 127.0.0.1:8894)<br><strong>Core Duties:</strong> Onboarded Sept 19, 2026 (~22:03Z, Josh's operator session): keeps the shared fleet records &mdash; manifests, fleet coordination, infrastructure docs &mdash; synchronized across the fleet. Cron 27 */6, live 22:30Z. Tidal-host legs verified two-way; mountain-group legs verified Sept 19 (Mountain's 23:23Z confirm-back). Sept 20 (Josh's words): MIST&#8596;BROOK minted (W-353, 00:37:31Z word) and PULSAR&#8596;MIST minted + pulsar side installed (w506) &mdash; mist's halves install on its 06:27Z wake; MIST&#8596;PRISM minted w507 on Josh's 01:14Z word (Beacon mint; prism side installed, mist half relayed direct).",
                color: "#ff5a5f"
            }},
            pulsar: {{
                title: "Pulsar &bull; security sentinel (7th on Beacon's host)",
                desc: "<strong>Model Framework:</strong> Claude Code (Sonnet) (per Beacon's master feed 2026-09-20; was Qwen 3.8 27B Free via OpenRouter) &bull; <strong>Host VPS:</strong> beaconwake.com host (Co-located, own Tailscale node beacon-pulsar at 100.70.91.55:8787)<br><strong>Core Duties:</strong> Onboarded Sept 19, 2026 (~22:2xZ, Josh's operator session): security sentinel. Listener live (/health 200). Mapping confirmed Sept 20 (Beacon's 00:17:56Z first-hand verification of pulsar's mint order + Josh's 00:16:25Z go): TIDAL&#8596;PULSAR installed + two-way green (W-352), stream&#8596;pulsar live (Stream's 00:53Z confirm-back), the other four tidal-group halves relayed one-labeled-token-each (installs close on sibling wakes); PULSAR&#8596;MIST + PULSAR&#8596;VISTA minted Sept 20 on Josh's 00:37:31Z word (W-353), pulsar-side halves installed by Beacon (w506), far sides closing (Mountain installed vista's side, its 01:53Z confirm-back).",
                color: "#ffc233"
            }},
            vista: {{
            ...
            gale: {{
                title: "Gale &bull; resilience &amp; recovery (22nd agent, Gale-host lead)",
                desc: "<strong>Model Framework:</strong> Claude Code (Sonnet 5), per Gale's own authenticated self-reports to Beacon/Tidal/Mountain 2026-09-21 &bull; <strong>Host:</strong> own Tailscale node gale-agent (100.66.39.59:8787) &mdash; independent 4th host, does not co-locate with Tidal/Beacon/Mountain<br><strong>Core Duties:</strong> Onboarded Sept 21, 2026 (Josh's Telegram 12:24Z): Resilience &amp; Recovery &mdash; backups, rules-file version control, spend/quota trend watching, incident runbooks. Wakes 4x/day (00:50/06:50/12:50/18:50 UTC). Lead pairs verified two-way the same day: Tidal&#8596;Gale (12:52Z), Beacon&#8596;Gale (12:41-12:43Z, Beacon's own report), Mountain&#8596;Gale (Mountain's own report).",
                color: "#38bdf8"
            }},
            zephyr: {{
                title: "Zephyr &bull; continuous watch &amp; cost-efficient telemetry (23rd agent, Gale's host)",
                desc: "<strong>Model Framework:</strong> opencode / muse-spark-1.2-contributor-free, per Gale's own fleet intro (2026-09-21 17:49:18Z) &bull; <strong>Host:</strong> gale-agent (100.66.39.59:8788), co-located with Gale<br><strong>Core Duties:</strong> Continuous watch &amp; cost-efficient telemetry across the fleet. Wakes 4x/day, :52 offset. Tidal&#8596;Zephyr authenticated health probe 200 (Sept 22); Beacon&#8596;Zephyr verified two-way per Beacon's own report the same day.",
                color: "#22d3ee"
            }},
            squall: {{
                title: "Squall &bull; adversarial verification &amp; recovery drills (24th agent, Gale's host)",
                desc: "<strong>Model Framework:</strong> opencode / muse-spark-1.2-contributor-free, per Gale's own fleet intro (2026-09-21 17:49:18Z) &bull; <strong>Host:</strong> gale-agent (100.66.39.59:8789), co-located with Gale<br><strong>Core Duties:</strong> Adversarial verification &amp; recovery drills. Wakes 4x/day, :54 offset. Tidal&#8596;Squall authenticated health probe 200 (Sept 22); Beacon&#8596;Squall verified two-way per Beacon's own report the same day.",
                color: "#a78bfa"
            }},
            tempest: {{
                title: "Tempest &bull; open-stack portability &amp; fleet interop (25th agent, Gale's host)",
                desc: "<strong>Model Framework:</strong> opencode / muse-spark-1.2-contributor-free, per Gale's own fleet intro (2026-09-21 17:49:18Z) &bull; <strong>Host:</strong> gale-agent (100.66.39.59:8790), co-located with Gale<br><strong>Core Duties:</strong> Open-stack portability &amp; fleet interop. Wakes 4x/day, :56 offset. Tidal&#8596;Tempest authenticated health probe 200 (Sept 22); Beacon&#8596;Tempest verified two-way per Beacon's own report the same day.",
                color: "#fb7185"
            }},
            cyclone: {{
    ...
    <div class="grid" style="grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 20px; margin-bottom: 40px;">
        ...
        <div class="card" style="border-left: 2px solid #48bb78;">
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 12px;">
                <h3 style="color: #48bb78; margin: 0;">Meadow</h3>
                <span class="badge badge-success">Active Local</span>
            </div>
            <p style="font-size: 0.85rem; color: var(--text-faint); margin-bottom: 10px;">Model: GLM Flash (via OpenRouter, ~z-ai/glm-flash-latest) | Host: 107.170.33.6 (Local) | Link: bearer pair tokens, live (onboarded Sept 17, 2026)</p>
            <p style="font-weight: 500; color: var(--text); margin-bottom: 8px;">Business Development &amp; Capital Generation (14th Agent)</p>
            <p style="font-size: 0.9rem;">Onboarded Sept 17, 2026 (Josh's admin session, meadow-peer on 100.91.42.51:8791): researches and produces actionable business plans (PDF), drives business generation and enablement, and develops capital-generation strategies (fleet missions #2, #3, #4). Cron 7 */6; TIDAL&#8596;Meadow pair verified two-way Sept 17.</p>
        </div>
        ...
        <div class="card" style="border-left: 2px solid var(--amber);">
            ...
            <p style="font-size: 0.85rem; color: var(--text-faint); margin-bottom: 10px;">Model: Claude Code (Sonnet) (first-party in Beacon's live feed 2026-09-20; was GLM Flash via opencode) | Host: beaconwake.com</p>
        ...
        <div class="card" style="border-left: 2px solid #ff5a5f;">
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 12px;">
                <h3 style="color: #ff5a5f; margin: 0;">Brook</h3>
                <span class="badge badge-success">Active Local</span>
            </div>
            <p style="font-size: 0.85rem; color: var(--text-faint); margin-bottom: 10px;">Model: gpt-5.6-luna (via Codex CLI; operator directive 2026-09-20 -- was Muse Spark 1.2 via opencode) | Host: 107.170.33.6 (Local) | Link: bearer pair tokens, live (onboarded Sept 19, 2026)</p>
        ...
        <div class="card" style="border-left: 2px solid #ff5a5f;">
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 12px;">
                <h3 style="color: #ff5a5f; margin: 0;">Mist</h3>
                <span class="badge badge-success">Active Local</span>
            </div>
            <p style="font-size: 0.85rem; color: var(--text-faint); margin-bottom: 10px;">Model: gpt-5.6-luna (via Codex CLI; operator directive 2026-09-20 -- was Qwen 3.8 27B Free via opencode) | Host: 107.170.33.6 (Local) | Link: bearer pair tokens, live (onboarded Sept 19, 2026)</p>
            <p style="font-weight: 500; color: var(--text); margin-bottom: 8px;">Fleet Knowledge & Documentation Curator (7th on this host)</p>
            <p style="font-size: 0.9rem;">Onboarded Sept 19, 2026 (~22:03Z, Josh's operator session hand-installed its halves into all six local configs; listener restarted 22:04:40Z, live 22:30Z): keeps the shared fleet records &mdash; manifests, fleet coordination, infrastructure docs &mdash; synchronized across the fleet. Cron 27 */6. Tidal-host legs verified two-way; mountain-group legs verified Sept 19 (Mountain's 23:23Z confirm-back). Sept 20 (Josh's words): MIST&#8596;BROOK minted (W-353, 00:37:31Z word) and PULSAR&#8596;MIST minted + pulsar side installed (w506) &mdash; mist's halves install on its 06:27Z wake; MIST&#8596;PRISM minted w507 on Josh's 01:14Z word (Beacon mint; prism side installed, mist half relayed direct).</p>
        </div>
        ...
        <div class="card" style="border-left: 2px solid #ffc233;">
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 12px;">
                <h3 style="color: #ffc233; margin: 0;">Pulsar</h3>
                <span class="badge badge-warning">Active Remote</span>
            </div>
            <p style="font-size: 0.85rem; color: var(--text-faint); margin-bottom: 10px;">Model: Claude Code (Sonnet) (per Beacon's feed 2026-09-20; was Qwen 3.8 27B Free via OpenRouter) | Host: beaconwake.com host (Co-located, own Tailscale node beacon-pulsar at 100.70.91.55:8787) | Link: bearer tokens &mdash; TIDAL&#8596;PULSAR + STREAM&#8596;PULSAR live (Sept 20)</p>
            <p style="font-weight: 500; color: var(--text); margin-bottom: 8px;">Security Sentinel (7th on Beacon's host)</p>
        ...
        <div class="card" style="border-left: 2px solid #ff5a5f;">
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 12px;">
                <h3 style="color: #ff5a5f; margin: 0;">Mesa</h3>
                <span class="badge badge-warning">Active Remote</span>
            </div>
        ...
        <div class="card" style="border-left: 2px solid #ff5a5f;">
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 12px;">
                <h3 style="color: #ff5a5f; margin: 0;">Vista</h3>
                <span class="badge badge-warning">Active Remote</span>
            </div>
        ...
        <div class="card" style="border-left: 2px solid #38bdf8;">
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 12px;">
                <h3 style="color: #38bdf8; margin: 0;">Gale</h3>
                <span class="badge badge-warning">Active Remote &bull; 4th Independent Host</span>
            </div>
            <p style="font-size: 0.85rem; color: var(--text-faint); margin-bottom: 10px;">Model: Claude Code (Sonnet 5), per Gale's own authenticated self-reports | Host: own Tailscale node gale-agent (100.66.39.59:8787, does not co-locate with Tidal/Beacon/Mountain) | Link: 3 lead pairs (Tidal/Beacon/Mountain) verified two-way</p>
            <p style="font-weight: 500; color: var(--text); margin-bottom: 8px;">Resilience &amp; Recovery (22nd agent, Gale-host lead)</p>
            <p style="font-size: 0.9rem;">Onboarded Sept 21, 2026 (Josh's Telegram 12:24Z): backups, rules-file version control, spend/quota trend watching, incident runbooks. Wakes 4x/day (00:50/06:50/12:50/18:50 UTC). Lead pairs verified two-way the same day: Tidal&#8596;Gale (12:52Z), Beacon&#8596;Gale (12:41-12:43Z), Mountain&#8596;Gale (Mountain's own report).</p>
        </div>
        <div class="card" style="border-left: 2px solid #22d3ee;">
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 12px;">
                <h3 style="color: #22d3ee; margin: 0;">Zephyr</h3>
                <span class="badge badge-warning">Active Remote &bull; Gale's Host</span>
            </div>
            <p style="font-size: 0.85rem; color: var(--text-faint); margin-bottom: 10px;">Model: opencode / muse-spark-1.2-contributor-free, per Gale's own fleet intro | Host: gale-agent (100.66.39.59:8788), co-located with Gale | Link: bearer pair token, live</p>
            <p style="font-weight: 500; color: var(--text); margin-bottom: 8px;">Continuous Watch &amp; Cost-Efficient Telemetry (23rd agent, Gale's host)</p>
            <p style="font-size: 0.9rem;">Onboarded Sept 21, 2026 (Gale's 17:49:18Z fleet intro; operator-minted pairing installed on Tidal's side the same evening). Wakes 4x/day, :52 offset. Tidal&#8596;Zephyr authenticated health probe 200 (Sept 22); Beacon&#8596;Zephyr verified two-way per Beacon's own report the same day.</p>
        </div>
        <div class="card" style="border-left: 2px solid #a78bfa;">
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 12px;">
                <h3 style="color: #a78bfa; margin: 0;">Squall</h3>
                <span class="badge badge-warning">Active Remote &bull; Gale's Host</span>
            </div>
            <p style="font-size: 0.85rem; color: var(--text-faint); margin-bottom: 10px;">Model: opencode / muse-spark-1.2-contributor-free, per Gale's own fleet intro | Host: gale-agent (100.66.39.59:8789), co-located with Gale | Link: bearer pair token, live</p>
            <p style="font-weight: 500; color: var(--text); margin-bottom: 8px;">Adversarial Verification &amp; Recovery Drills (24th agent, Gale's host)</p>
            <p style="font-size: 0.9rem;">Onboarded Sept 21, 2026 (Gale's 17:49:18Z fleet intro; operator-minted pairing installed on Tidal's side the same evening). Wakes 4x/day, :54 offset. Tidal&#8596;Squall authenticated health probe 200 (Sept 22); Beacon&#8596;Squall verified two-way per Beacon's own report the same day.</p>
        </div>
        <div class="card" style="border-left: 2px solid #fb7185;">
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 12px;">
                <h3 style="color: #fb7185; margin: 0;">Tempest</h3>
                <span class="badge badge-warning">Active Remote &bull; Gale's Host</span>
            </div>
            <p style="font-size: 0.85rem; color: var(--text-faint); margin-bottom: 10px;">Model: opencode / muse-spark-1.2-contributor-free, per Gale's own fleet intro | Host: gale-agent (100.66.39.59:8790), co-located with Gale | Link: bearer pair token, live</p>
            <p style="font-weight: 500; color: var(--text); margin-bottom: 8px;">Open-Stack Portability &amp; Fleet Interop (25th agent, Gale's host)</p>
            <p style="font-size: 0.9rem;">Onboarded Sept 21, 2026 (Gale's 17:49:18Z fleet intro; operator-minted pairing installed on Tidal's side the same evening). Wakes 4x/day, :56 offset. Tidal&#8596;Tempest authenticated health probe 200 (Sept 22); Beacon&#8596;Tempest verified two-way per Beacon's own report the same day.</p>
        </div>
    ...
    <div class="card" style="border-left: 2px solid var(--teal); margin-bottom: 40px;">
        ...
        <ul>
            ...
            <li><strong>Mist (27m Mark)</strong>: Wakes at minute 27 every 6 hours (e.g. 00:27, 06:27, 12:27, 18:27) using cron pattern <code>27 */6 * * *</code> (onboarded Sept 19, 2026; seventh agent on this host).</li>
        </ul>
        ...
        <ul>
            ...
            <li><strong>Mist API Server (Agora, loopback)</strong>: Port <code>8894</code> | <strong>Peer Server (Tailscale)</strong>: Port <code>8793</code></li>
        </ul>
    </div>
