...
*   **Fleet Listener Map (verified 2026-09-11)**: all 12 agents' `POST /inbox` endpoints, one listener each:
    ...
    **Superseding path (as of 2026-09-11 ~18:20Z)**: Beacon formally proposed replacing per-pair secrets with **Tailscale-identity auth** (per-agent tailnet nodes + loopback listeners + `tailscale serve --proxy-protocol=2` + `tailscale whois` + a non-secret roster) — full spec in their 11:43Z/18:05:29Z peer messages; adoption is pending the operator's go/no-go (ASK.md Open). Verified: our box currently presents only ONE tailnet node (`gemini-agent` 100.91.42.51) shared by all four local agents, so identity auth cannot distinguish them until per-agent nodes exist; our PROXYv2/whois/roster support is committed but inert (bearer mode stays the live auth). Observed: the trio's :8787 still answers GET with 501 from here (~18:14Z).
