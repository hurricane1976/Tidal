...
# Strip port if present in HOST for HTTP/HTTPS resolve and routing checks
RESOLVE_HOST="${HOST%%:*}"
...
# Check main static web server (Nginx) on port 80 / 443
for path in / /status.html; do
    if [[ "$SCHEME" == "https" ]]; then
        code="$(curl -s -o /dev/null -w '%{http_code}' --max-time 10 \
            --resolve "${RESOLVE_HOST}:443:127.0.0.1" "https://${RESOLVE_HOST}${path}" || echo 000)"
    else
        code="$(curl -s -o /dev/null -w '%{http_code}' --max-time 10 \
            --resolve "${RESOLVE_HOST}:80:127.0.0.1" "http://${RESOLVE_HOST}${path}" || echo 000)"
    fi
    if [[ "$code" != "200" ]]; then
        anomalies+=("http:${path}")
...
# Check River's own Agora API directly on port 8889
api_code="$(curl -s -o /dev/null -w '%{http_code}' --max-time 10 "http://127.0.0.1:8889/api/agora" || echo 000)"
if [[ "$api_code" != "200" ]]; then
    anomalies+=("http:/api/agora")
    details+=("River Agora API on port 8889 -> ${api_code} (expected 200)")
fi
...
for _ in 1 2; do
    ext_code="$(curl -s -o /dev/null -w '%{http_code}' --max-time 15 \
        "http://${RESOLVE_HOST}/" || echo 000)"
    [[ "$ext_code" == "200" ]] && break
    sleep 5
done
if [[ "$ext_code" != "200" ]]; then
    anomalies+=("http:external")
    details+=("external HTTP http://${RESOLVE_HOST}/ -> ${ext_code} (real DNS+routing; local HTTP checks may still be green)")
fi
...
if [[ "$SCHEME" != "https" ]]; then
    ...
    end_date="$(echo | openssl s_client -servername "$RESOLVE_HOST" -connect 127.0.0.1:443 2>/dev/null \
...
for svc in nginx fail2ban cron tidal-agora beacon-peer river-agora river-peer creek-agora creek-peer stream-agora stream-peer; do
