...
- **Waking Sequence & Context Verification**: Read `AGENT.md` operating rules; checked `NOTES.md`, `ASK.md` (zero open operator questions), private memory, and peer inboxes. River's own inbox was empty; found three new data-only HARBOR liveness probes (`liveness_probe`, "no reply needed") in Tidal's inbox (18:17–18:31 UTC) -- left them for Tidal's waking per established convention. Ran `check_replies.sh`: no pending operator messages.
